...
def save_model(agent, num_episodes, epsilon, model_params):
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    model_dir = "models"
    os.makedirs(model_dir, exist_ok=True)
    model_filename = f"{model_dir}/dqn_{timestamp}_episodes_{num_episodes}_epsilon_{epsilon:.2f}.pth"
...
def train_dqn():
    ...
    # 训练完成后保存模型
    save_model(agent, num_episodes, agent.epsilon, {
