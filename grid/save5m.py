...
# 文件名固定
output_file = "trade_suggestions_5m.csv"
...
def live_trading_suggestions(timeframe="1h"):
    ...
    if bb_upper_diff >= 0:
        bb_score = min(2.0, max(0.5, bb_upper_diff / last_bb_upper * 100))  # 远离上轨，卖出信号更强
        score -= bb_score
    ...
    print(
        f"RSI: {last_rsi}, MACD: {last_macd}, MACD_signal: {last_macd_signal}, SMA_50: {last_sma_50}, SMA_100: {last_sma_100}, 布林带上轨: {last_bb_upper}, 布林带下轨: {last_bb_lower}")
...
if __name__ == "__main__":
    while True:
        ...
        live_trading_suggestions(timeframe="5m")  # 每次获取1小时K线
        time.sleep(3600 / 4 / 3)  # 每小时更新一次
